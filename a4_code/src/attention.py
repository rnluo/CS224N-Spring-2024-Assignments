...
def precompute_rotary_emb(dim, max_positions):
    ...
    rope_cache = torch.stack((torch.cos(angle), torch.sin(angle)), dim=2)
...
def apply_rotary_emb(x, rope_cache):
    ...
    batch_size, n_head, seq_len, head_size = x.shape
    x_complex = torch.view_as_complex(torch.reshape(x, (batch_size, n_head, seq_len, head_size//2, 2)))
    rope_cache_complex = torch.view_as_complex(rope_cache[:seq_len])
    rotated_x = torch.reshape((torch.view_as_real(torch.mul(rope_cache_complex, x_complex))), x.shape)
