...
\begin{parts}
    ...
    \ifans{} The number of steps is $2n$, as each word is shifted once and removed from the stack once.
    ...
    \begin{enumerate}[label=\roman*.]
        ...
        \ifans{}
        \[\frac{\partial h_i}{\partial x_j} = \mathbb{I}[(xW + b_1)_i>0] \cdot \frac{\partial (xW)_i}{\partial x_j} = \mathbb{I}[(xW + b_1)_i] \cdot W_{ji}\]
        ...
        \ifans{}\\
        \begin{align*}
            \frac{\partial CE(y, \hat{y})}{\partial \bl_i} &= \sum_j \frac{\partial CE(y, \hat{y})}{\partial \hat{y}_j} \frac{\partial \hat{y}_j}{\partial \bl_i}\\
            &=\frac{\partial CE(y, \hat{y})}{\partial \hat{y}_c} \frac{\partial \hat{y}_c}{\partial \bl_i}\\
            &=-\frac{1}{\hat{y}_c} \frac{\partial \hat{y}_c}{\partial \bl_i}
        \end{align*} 
        \textit{Case 1.} $i=c$
            \begin{align*}
                -\frac{1}{\hat{y}_c} \frac{\partial \hat{y}_c}{\partial \bl_i} &= \hat{y}_i - 1
            \end{align*}       
        \textit{Case 2.} $i\neq c$
            \begin{align*}
                -\frac{1}{\hat{y}_c} \frac{\partial \hat{y}_c}{\partial \bl_i} &= \hat{y}_i
            \end{align*}
   ...
   In \texttt{parser\_model.py} you will find skeleton code to implement this simple neural network using PyTorch. Complete the \texttt{\_\_init\_\_}, \texttt{embedding\_lookup} and \texttt{forward} functions to implement the model. Then complete the \texttt{train\_for\_epoch} and \texttt{train} functions within the \texttt{run.py} file.
    ...
    \ifans{} Best dev UAS: 88.65, best test UAS: 88.99.
...
\ifans{} Using POS tags can disambiguate word roles and allow the parser to generalize syntactic patterns, making the parser more robust and improving dependency accuracy.
\end{parts}
