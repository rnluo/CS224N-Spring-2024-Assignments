...
\begin{parts}
    ...
    \begin{subparts}
        ...
        \ifans{}
        \begin{align*}
            \bH_{\textrm{perm}} &= \textrm{softmax}\left(\frac{\bP \bX \bW_Q (\bP \bX \bW_K)^\top}{\sqrt{d}}\right) \bP \bX \bW_V\\
            &= \bP \cdot \textrm{softmax}\left(\frac{\bQ \bK}{\sqrt{d}}\right) \bP^\top \bP \bV\\
            &= \bP \bH\\
            \bZ_{\textrm{perm}} &= \textrm{ReLU}(\bP \bH \bW_1 + \b1 \cdot \bb_1) \bW_2 + \b1 \cdot \bb_2\\
            &= \bP \cdot \textrm{ReLU}(\bH \bW_1 + \b1 \cdot \bb_1) \bW_2 + \bP \cdot \b1 \cdot \bb_2\\
            &= \bP \bZ
        \end{align*}
        ...
        \ifans{} There is no inherent notion of order. The model cannot capture the sequential information and positional dependence in text, which makes it insufficient for language processing because language heavily relies on word order to convey meaning.
    \end{subparts}
    ...
    \begin{subparts}
        ...
        \ifans{} Yes. Since the positional information are now encoded in input word embeddings, the model can distinguish tokens by position and learn to process them based on their order.
        ...
        \ifans{} Yes, but only for impractically large sequence lengths.
        For instance, 
        \[
        \Phi_{(t_1, 2i)} = \sin(\frac{t_1}{10000^{2i/d}}) = \sin(\frac{t_2}{10000^{2i/d}}) = \Phi_{(t_2, 2i)}
        \]
        where $t_2 - t_1$ is the least common multiple of $\{2\pi \cdot 10000^{0/d}, \ldots, 2\pi \cdot 10000^{2(d/2 - 1)/d}\}$.
        This far exceeds any reasonable sequence length.
    \end{subparts}
\end{parts}
