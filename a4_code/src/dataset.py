...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        masked_len = random.randint(1 * len(truncated_document) // 8, 3 * len(truncated_document) // 8 )
        masked_idx = random.randint(1, len(truncated_document) // 2)
        prefix = truncated_document[:masked_idx]
        masked_content = truncated_document[masked_idx: masked_idx + masked_len]
        suffix = truncated_document[masked_idx + masked_len:]

        masked_string = (prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content).ljust(self.block_size + 1, self.PAD_CHAR)
