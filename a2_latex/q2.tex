    ...
    \part[4] Adam Optimizer\newline
        ...
        \begin{subparts}
            ...
            \subpart[2]First, Adam uses a trick called {\it momentum} by keeping track of $\bm$, a rolling average of the gradients:
                ...
                \ifans{} Using $m$ ensures that the direction and distance of every update depends on the previous update.
                This may reduce oscillation in steep regions and preserve inertia to accelerate convergence in shallow regions.
            ...
            \subpart[2] Adam extends the idea of {\it momentum} with the trick of {\it adaptive learning rates} by keeping track of  $\bv$, a rolling average of the magnitudes of the gradients:
                ...
                \ifans{} The parameters with consistently smaller gradients will get larger updates.
                This allows the learning rates of parameters to adapt individually, preventing the parameters with large gradients from dominating the optimization and overshadowing parameters with small gradients. It also prevents overly large or unstable updates from noisy gradients.
            ...
            \subpart[2]
                ...
                \ifans{} 
                    \begin{align*}
                        \mathbb{E}_{p_{drop}}[\bh_{drop}]_i &= \mathbb{E}_{(\gamma \bd \odot \bh )_i}\\
                        &= \gamma (p_{drop} \cdot 0 + (1-p_{drop}) \cdot 1) h_i = h_i\\
            ...
            \subpart[2] Why should dropout be applied during training? Why should dropout \textbf{NOT} be applied during evaluation? \textbf{Hint:} it may help to look at the dropout paper linked. \newline
                ...
                \ifans{}
                \dots training a neural network with dropout can be seen as training a collection of 2n thinned networks with extensive weight sharing \dots
                This ensures that for any hidden unit the expected output (under the distribution used to drop units at training time) is the same as the actual output at test time \dots\\
