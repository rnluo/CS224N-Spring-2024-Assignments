...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        X_conv = self.post_embed_cnn(X)
        X = torch.permute(X_conv, (2, 0, 1))
        enc_hiddens_packed, (last_hidden, last_cell) = self.encoder(torch.nn.utils.rnn.pack_padded_sequence(X, source_lengths))
        enc_hiddens, _ = torch.nn.utils.rnn.pad_packed_sequence(enc_hiddens_packed, batch_first=True)
